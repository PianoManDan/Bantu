...
AudioInputI2S            audioInput;     //xy=109.16667175292969,132 (grabs digital audio)
AudioAnalyzeRMS          rms_L;          //xy=779.1666870117188,133
AudioAnalyzePeak         peak_L;         //xy=781.1666870117188,89.00000762939453
AudioConnection          patchCord1(audioInput, 0, peak_L, 0); //(source, sourcePort, destination, destinationPort)
...
elapsedMillis fps; //automatically increases as time elapses
elapsedMillis noAction;

uint8_t cnt=0; //8 bit unsigned number
int average[1000]; 
int averageCount =0;
int averageTotal =0;
int dot;
...
long currentDistance = 0; //32 bit
//   int average = 0;

int currentPosition  = 0; //returns the current motor position in steps. Positive is clockwise from the 0 position. 
...
//  }
 ...
 homeLimit();
 AudioMemory(6); //allocates memory for all audio connections. Sort of guess and check using AudioMemoryUsageMax()
 audioShield.enable();
...
void loop(){
   if(fps > 50) { //if 50 millis have passed since last time we ran this 
    update_analog(); //grabs the pot values
    if (peak_L.available() && rms_L.available()) { //lets you know if there's anything to read
